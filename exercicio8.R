# Dado o vetor com valores de medidas de comprimento em metros v<-c(20,30,4,934,1077,22),
# submeta a função paste() que apenas adiciona a unidade metro (‘m’) após cada número do 
# vetor. Note que não pode haver espaço entre o número e a unidade de medida. Além disso,
# use aspas simples e remova todos os espaços em banco do comando
...
# Note que tem que ser copiado e colado exatamente como está no site. Calcule e submeta a
# quantidade de caracteres totais, incluindo os espaços em branco e as quebras de linha(‘\n’). 
...
grep(pattern="(c|f|m)an", txt, value= TRUE)

# Dado a string txt = "eeer3GHJca1nfgSigmanfaUIOntafan", qual a expressão regular que deve
# ser adicionada no atributo ‘pattern’ da função str_extract_all(string=string,pattern=" ")
# para que ela extraia apenas números e letras maiúsculas. Note que você precisa carregar o
# pacote stringr. Também use o Regex.
...
# Dado a string txt = "tttyUFRPE19PPGIAkjgkjghk2j34j321UFRPEcapdPPGIA", qual a expressão 
# regular que deve ser adicionado no atributo ‘pattern’ da função 
# str_extract_all(string=txt,pattern=" "), para que ela extraia os padrões "UFRPE19PPGIA" 
# e "UFRPEcapdPPGIA".
...
# http://www.dhnet.org.br/desejos/sonhos/dream.htm . Qual a expressão regular trás 
...
str_extract_all(pattern="(n|N)egr(a|o)s?", string=txt)

# Qual das opções abaixo correspondem a regexp "a(ab)*a" 
grep(pattern="a(ab)*a", c("abababa", "aaba", "aabbaa", "aba","aabababa"), value = T)

# Qual das opções abaixo correspondem a regexp "ab+c?" ? 
grep(pattern="ab+c?", c("abc","ac","abbb","bbc"), value = T)

# Qual das opções abaixo correspondem a regexp "a.[bc]+" ?
grep(pattern = "a.[bc]+", c("abc", "abbbbbbbb", "azc", "abcbcbcbc", "ac", "asccbbbbcbcccc"), value = T)

# Qual das opções abaixo correspondem a regexp "abc|xyz" ?
grep(pattern="abc|xyz", c("abc","xyz","abc|xyz"), value = T)

# Qual das opções abaixo correspondem a regexp "[a-z]+[\.\?!]" ?
grep(pattern="[a-z]+[\\.\\?!]", c("battle!", "Hot", "green", "swamping.", "jump up.", "undulate?", "is.?"), value = TRUE)

# Qual das opções abaixo correspondem a regexp "[a-zA-Z]*[^,]=" ?
grep(pattern="[a-zA-Z]*[^,]", c("Butt=", "BotHEr,=", "Ample", "FIdDlE7h=", "Brittle =", "Other.="), value=T)

# Qual das opções abaixo correspondem a regexp "[a-z][\.\?!]\s+[A-Z]" ?
grep(pattern="[a-z][\\.\\?!]\\s+[A-Z]", c("A. B", "c! d", "e f", "g. H", "i? J", "k L"), value = T)

# Qual das opções abaixo correspondem a regexp "(very )+(fat )?(tall|ugly) man" ?
grep(pattern="(very )+(fat )?(tall|ugly) man", c("very fat man", "fat tall man", "very very fat ugly man", "very very very tall man"), value = T)

# Qual das opções abaixo correspondem a regexp "<[^>]+>" ?
grep(pattern="<[^>]+>", c("<an xml tag>", "<opentag> <closetag>", "</closetag>", "<>", "<with attributes=\"77\">"), value=T)
...
# Usando quantificadores, encontre todos os países com ee, mas não com eee, em seu nome.
